...
export default function ListaObecnosci({ probaId, sekcja }) {
  ...
  const [daneObecnosci, setDaneObecnosci] = useState({}); // userId -> { planuje, usprawiedliwienie, obecny }
  ...
  const pobierzOsobyIDeklaracje = async () => {
    ...
    // Pobieramy deklaracje ORAZ realną obecność
  ...
  const oznaczObecnosc = async (userId, statusObecnosci) => {
    ...
    // Zapisujemy w bazie faktyczną obecność, zachowując deklarację bez zmian
  ...
  const renderujWpisOsoby = (osoba, isGosc = false) => {
    ...
    return (
      <li key={osoba.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', backgroundColor: isGosc ? '#faf5ff' : '#f8fafc', borderRadius: '6px', border: isGosc ? '1px solid #f3e8ff' : '1px solid #e2e8f0', flexWrap: 'wrap', gap: '10px' }}>
        ...
        {/* Dane członka i jego DEKLARACJA */}
        ...
        {/* PRZYCISKI DO SPRAWDZANIA REALNEJ OBECNOŚCI PRZEZ KADRĘ */}
        ...
        ['Sopran', 'Alt', 'Tenor', 'Bas'].map(glosName => {
          ...
          return (
            <div key={glosName} style={{ marginBottom: '15px' }}>
              <p style={{ fontSize: '12px', fontWeight: 'bold', color: '#d97706', margin: '0 0 6px 0', textTransform: 'uppercase' }}>{glosName} ({osobyGlosu.length}):</p>
