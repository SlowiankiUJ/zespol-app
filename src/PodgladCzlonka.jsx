...
export default function PodgladCzlonka({ profile }) {
  ...
  const pobierzDodatkoweSekcje = async () => {
    const { data, error } = await supabase.from('dodatkowe_sekcje').select('*').eq('id_uzytkownika', profile.id);
  ...
  const zaktualizujProfil = async (e) => {
    ...
    const { error } = await supabase.from('profiles').update(daneDoAktualizacji).eq('id', profile.id);
    setLoading(false);

    if (error) setKomunikat('Błąd: ' + error.message);
    else {
  ...
  const wgrajZdjecie = async (event) => {
    try {
      ...
      // 1. Wrzucenie pliku do bucketu 'avatars'
      let { error: uploadError } = await supabase.storage.from('avatars').upload(filePath, file);
      if (uploadError) throw uploadError;

      // 2. Pobranie publicznego linku do pliku
      const { data } = supabase.storage.from('avatars').getPublicUrl(filePath);

      // 3. Aktualizacja profilu o nowy link
  ...
  const wyslijProsteDoSekcji = async (e) => {
    e.preventDefault();
    if (dodatkowaSekcjaWybór === profile.sekcja) { alert('To jest Twoja główna sekcja!'); return; }
    const juzIstnieje = mojeDodatkoweSekcje.some(ds => ds.sekcja === dodatkowaSekcjaWybór);
    if (juzIstnieje) { alert('Masz już wysłaną prośbę lub dostęp do tej sekcji.'); return; }

    const { error } = await supabase.from('dodatkowe_sekcje').insert([{ id_uzytkownika: profile.id, sekcja: dodatkowaSekcjaWybór, status: 'oczekujacy' }]);
    if (error) alert('Błąd: ' + error.message);
    else { alert('Prośba wysłana! ⏳'); pobierzDodatkoweSekcje(); }
  };

  const usunDodatkowaSekcje = async (id) => {
    if (!window.confirm('Rezygnujesz z dodatkowej sekcji?')) return;
  ...
  return (
    <div style={{ marginTop: '20px', padding: '25px', border: '1px solid #e2e8f0', borderRadius: '12px', backgroundColor: '#ffffff', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', maxWidth: '600px', marginLeft: 'auto', marginRight: 'auto' }}>
      ...
      <div style={{ marginBottom: '25px', padding: '15px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
        <p style={{ margin: '0 0 8px 0', fontSize: '14px', color: '#334155' }}><strong>Rola:</strong> <span style={{ textTransform: 'capitalize', color: '#8b5cf6', fontWeight: 'bold' }}>{profile.rola}</span></p>
        <p style={{ margin: '0 0 8px 0', fontSize: '14px', color: '#334155' }}><strong>Główna sekcja:</strong> <span style={{ textTransform: 'uppercase', color: '#3182ce', fontWeight: 'bold' }}>{profile.sekcja}</span></p>
        {profile.sekcja === 'chór' && <p style={{ margin: 0, fontSize: '14px', color: '#334155' }}><strong>Głos:</strong> <span style={{ color: '#d97706', fontWeight: 'bold' }}>{profile.glos || 'Nie wybrano'}</span></p>}
      </div>
      ...
      <form onSubmit={zaktualizujProfil} style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '30px' }}>
        <div>
          ...
          <input type="text" value={imieNazwisko} onChange={(e) => setImieNazwisko(e.target.value)} required style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1' }} />
        </div>
        {profile.sekcja === 'chór' && (
          <div>
            <label style={{ display: 'block', fontSize: '14px', fontWeight: '600', color: '#334155', marginBottom: '5px' }}>Wybierz głos:</label>
            <select value={glos} onChange={(e) => setGlos(e.target.value)} required style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
      ...
      <h3 style={{ fontSize: '16px', color: '#1e293b', marginBottom: '10px' }}>Dodatkowe sekcje</h3>
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px 0', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {mojeDodatkoweSekcje.map(ds => (
            <li key={ds.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', backgroundColor: '#f8fafc', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
              <span style={{ fontSize: '14px', textTransform: 'uppercase', fontWeight: '600', color: '#334155' }}>{ds.sekcja} {ds.status === 'zatwierdzony' ? '🟢' : '⏳'}</span>
              <button onClick={() => usunDodatkowaSekcje(ds.id)} style={{ padding: '4px 8px', backgroundColor: '#ef4444', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>Rezygnuj ❌</button>
            </li>
          ))}
        </ul>
      )}
      <form onSubmit={wyslijProsteDoSekcji} style={{ display: 'flex', gap: '10px' }}>
        <select value={dodatkowaSekcjaWybór} onChange={(e) => setDodatkowaSekcjaWybór(e.target.value)} style={{ flex: 1, padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
          <option value="balet">Balet</option><option value="chór">Chór</option><option value="kapela">Kapela</option>
        </select>
        <button type="submit" style={{ padding: '8px 14px', backgroundColor: '#3182ce', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '13px' }}>Poproś o dostęp ➕</button>
      </form>
    </div>
  );
}